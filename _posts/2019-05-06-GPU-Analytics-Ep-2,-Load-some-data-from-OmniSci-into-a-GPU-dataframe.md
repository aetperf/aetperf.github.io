...
Although the post title is about loading some data from a GPU database into a GPU dataframe, most of it is about running [JupyterLab](https://jupyterlab.readthedocs.io/en/stable/) on a GPU AWS instance, which is a little bit cumbersome to set up. Finally, once JupyterLab is runnning on our `p3.2xlarge` instance, we are going to install [RAPIDS](https://rapids.ai/) cuDF and cuML libraries and perform a very short test.
...
By the way, each of these people happened to write a book that I found very interesting:

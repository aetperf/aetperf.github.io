...
using the power series expansion for $e^x$: 
$$e^x = \sum_{n=0}^{\infty} \frac{x^n}{n!}$$

So we finally got our theoretical result: $$m(1)=e$$
...
We can compare the approximated analytical values of $m(x)$ with the exact values from Wolfram MathWorld [[5]](https://mathworld.wolfram.com/UniformSumDistribution.html):
...
Quite close! Now that that we are rather confident about the accuracy of the approximation of $m(x)$, we can evaluate and plot it on a discretized segment $(0,6]$:
...
It makes sense since a single random uniform number is almost sure to be strictly larger than zero. We can zoom in the left part of the $m(x)$ to observe its shape.  
